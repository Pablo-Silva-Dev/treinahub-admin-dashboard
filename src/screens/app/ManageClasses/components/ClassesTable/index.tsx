...
import { IClass } from "@/interfaces/dtos/Class";
...
const TABLE_HEAD = [
  ...
  { label: "Módulo", propRef: "module_name" },
  { label: "Tutor", propRef: "tutor_name" },
...
export function ClassesTable({
  ...
  const [sortedClasses, setSortedClasses] = useState<IClass[]>([]);
  const [tableData, setTableData] = useState<IClass[]>([]);
  const [itemsPerPage, setItemsPerPage] = useState(
    itemsPerPageOptions[0].value
  );

  const pages = Array.from(
    { length: Math.ceil(sortedClasses.length / itemsPerPage) },
    (_, idx) => idx + 1
  );
  ...
  const currentTableData = useCallback(() => {
    ...
    setTableData(sortedClasses.slice(startIndex, endIndex) as never);
  }, [itemsPerPage, page, sortedClasses]);
                  ...
                  >
                    ...
                    <td className={classes}>
                      <div className="flex items-center gap-3">
                        <Subtitle
                          content={collapseLongString(name, MAX_STRING_LENGTH)}
                    ...
                    <td className={classes}>
                      <Text
                        ...
                        className="block overflow-hidden text-ellipsis whitespace-nowrap text-[11px] md:text[12px] lg:text-sm ml-2 lg:ml-4 min-w-[132px] text-gray-700 dark:text-gray-300"
                      />
                    </td>
                    <td className={classes}>
                      <Text
                        ...
                        className="block overflow-hidden text-ellipsis whitespace-nowrap text-[11px] md:text[12px] lg:text-sm ml-2 lg:ml-4 min-w-[96px] text-gray-700 dark:text-gray-300"
                      />
                    </td>
                    <td className={classes}>
                      <Text
              ...
              <div className="flex flex-row">
                ...
                {pages
                  ...
                  .map((ClassPage) => (
                    <IconButton
                      key={ClassPage}
                      variant={page === ClassPage ? "outlined" : "text"}
                      size="sm"
                      onClick={() => setPage(ClassPage)}
