...
import { ICourse } from "@/interfaces/dtos/Course";
...
export function CoursesTable({
  ...
  const handleSort = (propRef: string, sortType: "asc" | "desc") => {
    const sortedData = sortItems([...courses], propRef, sortType);
      ...
      <CardBody className="overflow-scroll p-0 rounded-lg w-full">
        <table className="w-full table-auto text-left">
          ...
          <tbody className="w-[90%] lg:w-full">
            {tableData.map(
              ({ name, description, duration, students }, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className={classes}>
                      <Text
                        content={duration}
                        className="text-[11px] md:text[12px] text-ellipsis lg:text-sm ml-2 lg:ml-4 text-gray-700 dark:text-gray-300"
                      />
                    </td>
                    <td className={classes}>
                      <Text
